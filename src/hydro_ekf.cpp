...
void HydroEKF::set_inputs(double Q_water_api, double p_now){
    _Q_water = Q_water_api;
    _p_actual = p_now;
}

VectorXd HydroEKF::f(const VectorXd& x, double dt){
    VectorXd x_new(2);
...
MatrixXd HydroEKF::F(const VectorXd& x, double dt){
    MatrixXd Fj(2, 2);
    double w = std::max(x(0), 0.1);
    double p_max = x(1);
...
VectorXd HydroEKF::h(const VectorXd& x_pred){
    VectorXd z_pred(1);
...
MatrixXd HydroEKF::H(const VectorXd& x){
    MatrixXd Hj(1, 2);
